...
for i in range(0, 67):  # Put the amount of pictures you have taken for the calibration inbetween range(0,?) wenn starting from the image number 0
    t = str(i)
    ChessImaR = cv2.imread('capture_img/chessboard-R' + t + '.png', 0)  # Right side
    ChessImaL = cv2.imread('capture_img/chessboard-L' + t + '.png', 0)  # Left side
...
while True:
    ...
    original_frameR = frameR
    original_frameL = frameL
    ...
    cv2.imshow('Original FrameR', original_frameR)
    cv2.imshow('Original FrameL', original_frameL)
