'''
    How to use:
        ...
        python 2-calibrate.py capture/left capture/right calibration
...
def readImagesAndFindChessboards(imageDirectory):
    ...
    for imagePath in sorted(imagePaths):
        ...
        hasCorners, corners = cv2.findChessboardCorners(grayImage, CHESSBOARD_SIZE, cv2.CALIB_CB_FAST_CHECK)
...
if (len(filenames) > MAX_IMAGES):
    print("Too many images to calibrate, using {0} randomly selected images".format(MAX_IMAGES))
...
_, leftCameraMatrix, leftDistortionCoefficients, _, _ = cv2.calibrateCamera(objectPoints,
                                                                            leftImagePoints,
                                                                            imageSize,
                                                                            None,
                                                                            None)
# leftDistortionCoefficients = np.array([[leftDistortionCoefficients[0][0]], [leftDistortionCoefficients[0][1]], [0], [0], [0]])

print("Calibrating right camera...")
_, rightCameraMatrix, rightDistortionCoefficients, _, _ = cv2.calibrateCamera(objectPoints,
                                                                              rightImagePoints,
                                                                              imageSize,
                                                                              None,
                                                                              None)
# rightDistortionCoefficients = np.array([[rightDistortionCoefficients[0][0]], [rightDistortionCoefficients[0][1]], [0], [0], [0]])

print("Calibrating cameras together...")
(_, _, _, _, _, rotationMatrix, translationVector, _, _) = cv2.stereoCalibrate(objectPoints,
                                                                               leftImagePoints,
                                                                               rightImagePoints,
                                                                               leftCameraMatrix,
...
(leftRectification, rightRectification, leftProjection, rightProjection, dispartityToDepthMap, leftROI, rightROI) = cv2.stereoRectify(leftCameraMatrix,
                                                                                                                                      leftDistortionCoefficients,
                                                                                                                                      rightCameraMatrix,
                                                                                                                                      rightDistortionCoefficients,
                                                                                                                                      imageSize,
...
leftMapX, leftMapY = cv2.initUndistortRectifyMap(leftCameraMatrix,
                                                 ...
                                                 imageSize,
                                                 cv2.CV_32FC1)
rightMapX, rightMapY = cv2.initUndistortRectifyMap(rightCameraMatrix,
                                                   rightDistortionCoefficients,
                                                   rightRectification,
                                                   rightProjection,
                                                   imageSize,
